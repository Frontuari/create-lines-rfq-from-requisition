...
public class WCreateRfqFromRequisitionUI extends CreateFrom implements EventListener<Event>
{
	
	private WCreateFromWindow window;
	
	public WCreateRfqFromRequisitionUI(GridTab tab) 
	{
		super(tab);
	...
	public boolean dynInit() throws Exception
	{
		...
		if (getGridTab().getValue("C_RfQ_ID") == null)
		{
	...
	//  Order
			int C_RfQ_ID = ((Integer)getGridTab().getValue("C_RfQ_ID")).intValue();
			MRfQ rfq = new MRfQ (Env.getCtx(), C_RfQ_ID, trxName);
			if (log.isLoggable(Level.CONFIG)) log.config(rfq.toString());
			...
			for (int i = 0; i < miniTable.getRowCount(); i++)
			{
				if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue())
				{
					...
					//	Create new Rfq Line
					MRfQLine m_rfqLine = new MRfQLine (rfq);
					//m_rfqLine.setDatePromised(rLine.getDateRequired());
					if (rLine.getM_Product_ID() >0)
					{
						m_rfqLine.setM_Product_ID(rLine.getM_Product_ID());
						m_rfqLine.setM_AttributeSetInstance_ID(rLine.getM_AttributeSetInstance_ID());
						m_rfqLine.setDescription(rLine.getDescription());
						//m_rfqLine.setPriceActual(rLine.getPriceActual());
						m_rfqLine.setAD_Org_ID(rLine.getAD_Org_ID());
						//m_rfqLine.setQty(rLine.getQty());
						m_rfqLine.set_ValueOfColumn("M_RequisitionLine_ID", rLine.get_ID());
						m_rfqLine.setLine(rLine.getLine());
						m_rfqLine.saveEx();
						
						//create quantity
						MRfQLineQty m_rfqlineqty = new MRfQLineQty(m_rfqLine);
						m_rfqlineqty.setBenchmarkPrice(rLine.getPriceActual());
						m_rfqlineqty.setQty(rLine.getQty());
						m_rfqlineqty.setC_UOM_ID(rLine.getC_UOM_ID());
						m_rfqlineqty.setIsPurchaseQty(rLine.getM_Product().isPurchased());
						m_rfqlineqty.setIsOfferQty(rLine.getM_Product().isSold());
						
						m_rfqlineqty.set_ValueOfColumn("M_RequisitionLine_ID", rLine.get_ID());
						m_rfqlineqty.saveEx();
					}
				}   //   if selected
			}   //  for all rows
	...
	protected Vector<Vector<Object>> getRequisitionData(Object Requisition, Object Org,  Object User)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				boolean add = line.add(new Boolean(false));           //  0-Selection
